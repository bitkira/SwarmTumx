...
- Wake delivery uses a stable sentinel string after a quiet screen window.

The sentinel is:

`[SWARMTUMX_NOTIFY] pending inbox items; use read_inbox`

Treat that sentinel as a wake signal only.
...
5. When the sentinel appears, run `swarmtumx-agent read_inbox`.
...
- Never treat the sentinel as actual message content.
...
- Run `swarmtumx-agent read_inbox`.
